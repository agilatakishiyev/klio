...
# TODO: should this be removed? I think I might have added it because I got annoyed
...
from apache_beam.runners.direct import consumer_tracking_pipeline_visitor as ctpv
    ...
    def run_pipeline(self, pipeline, options):
        ...
        class TestStreamUsageVisitor(beam_pipeline.PipelineVisitor):
            ...
            def visit_transform(self, applied_ptransform):
                if isinstance(applied_ptransform.transform, test_stream.TestStream):
                    self.uses_test_stream = True

        visitor = TestStreamUsageVisitor()
        pipeline.visit(visitor)
        clock = beam_clock.TestClock() if visitor.uses_test_stream else beam_clock.RealClock()
        ...
        bndl_factory = bundle_factory.BundleFactory(
            stacked=options.view_as(pipeline_options.DirectOptions).direct_runner_use_stacked_bundle
        )
        ...
        executor = beam_exec.Executor(
            self.consumer_tracking_visitor.value_to_consumers,
            pubsub_message_manager.KlioTransformEvaluatorRegistry(evaluation_context),
            evaluation_context)
